...
$hasError = false;
if ($name) {
    $hasError = $errors->has($name) && !$errorless;
}
@endphp
...
<div class="@if ($disabled) opacity-60 @endif">
    @if ($label || $cornerHint)
        <div class="flex {{ !$label && $cornerHint ? 'justify-end' : 'justify-between' }} mb-1">
            @if ($label)
                <x-dynamic-component :component="WireUiComponent::resolve('label')"
                    :label="$label"
                    :has-error="$hasError"
                    :for="$id" />
            @endif
    ...
    <div class="relative rounded-md @unless($shadowless) shadow-sm @endunless">
        @if ($prefix || $icon)
            <div
                ...
                @if ($icon)
                    <x-dynamic-component :component="WireUiComponent::resolve('icon')"
                        :name="$icon"
                        class="h-5 w-5" />
                @elseif($prefix)
    ...
    @if ($name && !$errorless)
        <x-dynamic-component :component="WireUiComponent::resolve('error')"
            :name="$name" />
    @endif
</div>
